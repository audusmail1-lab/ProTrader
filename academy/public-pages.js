...
export function privacy(policy, telegram = {}) {
  return page('Privacy Policy', 'How the academy uses your information, who helps us run it, and how to contact us about your data.', `
    ...
    <h2>Website support chat</h2><p>We store support messages and timestamps to respond and follow up. Signed-in conversations are linked to your Academy account; guests use an essential private cookie that expires after seven days. Keep the same browser to receive guest replies. Authorized instructors can read and answer these conversations.</p><p>If AI help is enabled and you choose it, relevant recent chat messages are sent to OpenAI to generate an answer. You can choose a human instead. We request that API responses are not stored for later retrieval; provider security and abuse-monitoring retention may still apply. Never send passwords, API keys, financial details or identity documents.</p><p>Closed website support conversations follow the support retention period above. Contact us for access or deletion requests. Human replies appear in the same website chat; verified account holders are also notified by email. This is not a guarantee that an agent is online.</p>
